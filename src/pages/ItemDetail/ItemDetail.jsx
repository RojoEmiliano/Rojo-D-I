...
const ItemDetail = () => {
  ...
  const [item, setItem] = useState(() => {
    if (location.pathname.includes('ricoh')) {
      return ricohProducts.filter(product => product.specs.modelCode === modelCode)[0];
    } else if (location.pathname.includes('epson')) {
      return epsonProducts.filter(product => product.specs.modelCode === modelCode)[0];
    }
  });
  ...
  return (
    <>
      <HelmetCommon
        title={`ROJO D-I: ${
          location.pathname.includes('ricoh')
            ? ricohProducts.filter(product => product.specs.modelCode === modelCode)[0].name
            : epsonProducts.filter(product => product.specs.modelCode === modelCode)[0].name
        }`}
      />
        ...
        <div className='mt-4 p-4 bg-gray-100 shadow-md'>
          <div className='flex lbMax:flex-col lbMax:items-center sb:px-10 justify-between'>
            <div className='lb:w-1/2 lb:max-w-[700px] sb:max-w-[500px] sbMax:max-w-[350px] h-fit bg-white'>
              ...
              <SliderComponent slides={item.imgs} theme={theme} dots={true} />
              <div className='h-7 bg-white'></div>
            </div>
            <div className='lb:w-[45%] lbMax:max-w-[700px] flex flex-col lbMax:mt-4'>
              {!useMediaQuery(1024) && (
                <div className='text-3xl font-semibold mb-4'>{item.name}</div>
              )}
                ...
                <Accordion.Body>
                  ...
                  <div className='py-4'>
                    ...
                    <li>
                      ...
                      {item.specs.general.os}
                    </li>
                    <li>
                      <span className='font-semibold'>Tamaño:</span> {item.specs.general.size}
                    </li>
                    <li>
                      <span className='font-semibold'>Peso:</span> {item.specs.general.weight}
                    </li>
                    <li>
                      <span className='font-semibold'>Origen:</span> {item.specs.general.origin}
                    </li>
                  </div>
                  <hr />
                  <div className='py-4'>
                    ...
                    <li>
                      ...
                      {item.specs.conectivity.standarConectivity}
                    </li>
                    <li>
                      <span className='font-semibold'>Protocolos de Red:</span>{' '}
                      {item.specs.conectivity.netProtocols}
                    </li>
                  </div>
                  <hr />
                  <div className='py-4'>
